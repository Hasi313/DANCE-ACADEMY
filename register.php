...
if(isset($POST['submit'])){
	...
	$mobile = mysqli_real_escape_string ($conn, $POST['mobile']);
	$password = mysqli_real_escape_string($conn, sha1($POST['password']));
	$conf_password = mysqli_real_escape_string($conn, sha1($POST['conf_password']));
	...
	if(mysqli_num_rows($getUser) > 0){
		...
		if($password != $conf_password){
			...
			$sql = "INSERT INTO tbl_users(username, email, mobile, password, use_role_id) VALUES('$username', '$email', '$mobile', '$password', '$user_role_id')";
					...
					<form method="post" action="">
						...
						<div class="form-group">
							...
							if(mysqli_num_rows($getRoles) > 0){ ?>
								...
								<option values="3">Student</option>
							<?php }else{?>
